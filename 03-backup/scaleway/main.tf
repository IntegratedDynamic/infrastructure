...
# Separate bucket for Velero (gitops repo platform/scaleway/velero.yml), not a
# prefix inside the bucket above: confirmed live (2026-07-28) that OpenBao's
# snapshot script does a flat `s3cmd ls` on the bucket root for its own
# retention cleanup and chokes on any object/prefix it doesn't own — Velero
# writing into a `velero/` prefix inside this same bucket broke every
# subsequent OpenBao snapshot job. Scaleway's IAM can't scope Object Storage
# permissions below project level anyway (see iam.tf), so a separate bucket —
# not a separate IAM policy — is what actually isolates the two.
...
resource "scaleway_object_bucket_server_side_encryption_configuration" "velero" {
  bucket = scaleway_object_bucket.velero.name
  region = var.region
